  ...
  <div id="app">
    <div class="content">
      <select multiple data-multi-select-plugin>
		<option :value="word" v-for="(word, index) in words" :key="index">{{word}}</option>
      </select>
    </div>
  </div>
</template>
...
export default {
	...
	methods: {
		...
		createInitialTokens(select) {
			let { options_selected } = this.getOptions(select);
			const wrapper = select.parentNode;
			for (let i = 0; i < options_selected.length; i++) {
				this.createToken(wrapper, options_selected[i]);
			}
		},
		...
		inputChange(e) {
			...
			this.populateAutocompleteList(select, input_val.trim());
			} else {
				dropdown.classList.remove("active");
		...
		clickOnWrapper(e) {
			...
			if (wrapper.tagName == "DIV") {
				...
				input_search.focus();
				this.removePlaceholder(wrapper);
			}
		},
		openOptions(e) {
		...
		createToken(wrapper, value) {
			...
			close.innerText = "x";
			close.addEventListener("click", this.removeToken)
		...
		clickDropdown(e) {
			...
			if (dropdown.classList.contains("active")) {
				...
				if (!input_search.value) {
					this.populateAutocompleteList(select, "", true);
				} else {
					this.populateAutocompleteList(select, input_search.value);

				}
			} else {
				this.clearAutocompleteList(select);
				this.addPlaceholder(wrapper);
			}
		},
		...
		populateAutocompleteList(select, query, dropdown = false) {
			const { autocomplete_options } = this.getOptions(select);
			...
			if (dropdown)
				...
				options_to_show = this.autocomplete(query, autocomplete_options);
		...
		selectOption(e) {
			...
			this.createToken(wrapper, e.target.dataset.value);
			if (input_search.value) {
				input_search.value = "";
			}
		...
		getOptions(select) {
			...
			return { options_selected, autocomplete_options};
		},
		// Listener for when the user wants to remove a given token.
		removeToken(e) {
			// Get the value to remove
			this.keywords.splice(this.keywords.indexOf(e.target.dataset.option), 1);
